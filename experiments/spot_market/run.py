"""Runner for spot_market.
    ...
    python -m experiments.spot_market.run --agent-overrides model_configs/one_seller_gemma.json
        # give ONE specific agent a different model and/or reasoning_effort, everyone
        # else stays on the env default -- see experiments/spot_market/model_configs/
"""
...
async def main() -> None:
    ...
    ap.add_argument("--agent-overrides", default=None,
                    help="path to a JSON file mapping specific agent ids to a model and/or "
                    "reasoning_effort, e.g. {\"S1\": {\"reasoning_effort\": \"high\"}} or "
    ...
    agent_overrides = {}
    if args.agent_overrides:
        agent_overrides = json.loads(Path(args.agent_overrides).read_text())
